...
class InferenceCore:
    ...
    def do_pass(self, key_k, key_v, idx, end_idx, mask):
        ...
        pre_mask = mask
        #palette = Image.open(path.expanduser('/mnt/31f271cb-1eab-41e4-aa15-7caf8b6e7528/gck/Dataset/YouTube_19/valid/Annotations/0a49f5265b/00000.png')).getpalette()
        for ti in this_range:
            is_mem_frame = (abs(ti - last_ti) >= self.mem_every)
            # Why even work on it if it is not required for memory/output
            if (not is_mem_frame) and (not self.include_last) and (self.req_frames is not None) and (
                    ti not in self.req_frames):
                continue
            ...
            out_mask = torch.cat([
                self.prop_net.segment_with_query(self.mem_banks[oi], qf8, qf4, k16, qv16, pre_mask[idx], key_k,
                                                 key_v[idx])
                for idx, oi in enumerate(self.enabled_obj)], 0)
    ...
    def interact(self, mask, frame_idx, end_idx, obj_idx):
        ...
        palette = Image.open(path.expanduser(
            '/mnt/31f271cb-1eab-41e4-aa15-7caf8b6e7528/gck/Dataset/YouTube_19/valid/Annotations/0a49f5265b/00000.png')).getpalette()
        # amask = mask[2]
        ...
        key_v = self.prop_net.encode_value(self.images[:, frame_idx].cuda(), qf16,
                                           self.prob[self.enabled_obj, frame_idx].cuda())
        key_k = key_k.unsqueeze(2)
        mask = self.prob[1:, frame_idx]
        # Propagate
