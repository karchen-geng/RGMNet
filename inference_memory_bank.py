...
class MemoryBank:
    ...
    def match_memory(self, qk):
        k = self.num_objects  # 2
        _, _, h, w = qk.shape  # 1，64，30，57

        qk = qk.flatten(start_dim=2)  # 1,64,1710(30*57)
        # mk不变
        if self.temp_k is not None:
            ...
            mk = self.mem_k
            mv = self.mem_v
        affinity = self._global_matching(mk, qk)  # 1,1710,1710
        # One affinity for all
        readout_mem = self._readout(affinity.expand(k, -1, -1), mv)

        return readout_mem.view(k, self.CV, h, w)

    def match_last(self, qk, key_k, key_v):
        k = self.num_objects  # 2
        _, _, h, w = qk.shape  # 1，64，30，57
        # youtube is required
        # if k == 1:
        #     key_v = key_v.unsqueeze(0)

        key_k = key_k.flatten(start_dim=2)
        key_v = key_v.flatten(start_dim=2)
        qk = qk.flatten(start_dim=2)  # 1,64,1710(30*57)

        affinity = self._global_matching(key_k, qk)  # 1,1710,1710
        readout_mem_last = self._readout(affinity.expand(k, -1, -1), key_v)  # 2*512*1710

        return readout_mem_last.view(k, self.CV, h, w)
