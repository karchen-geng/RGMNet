...
class STCNModel:
    ...
    def do_pass(self, data, it=0):
        #  rgb ,gt ,cls_gt ,sec_gt ,selector ,info:(name,frames)
        ...
        out = {}
        Fs = data['rgb']  # 4, 3, 3, 384, 384  batch=4
        Ms = data['gt']  # 4, 3, 1, 384, 384

        with torch.cuda.amp.autocast(enabled=self.para['amp']):
            # key features never change, compute once 计算所有的key一次
            # [4, 64, 3, 24, 24],,[4, 3, 512, 24, 24],,[4, 3, 1024, 24, 24],,[4, 3, 512, 48, 48],,[4, 3, 256, 96, 96])
            k16, kf16_thin, kf16, kf8, kf4 = self.STCN('encode_key', Fs)

            if self.single_object:
                ref_v = self.STCN('encode_value', Fs[:, 0], kf16[:, 0], Ms[:, 0])  # 0帧的value

                # Segment frame 1 with frame 0
                prev_logits, prev_mask = self.STCN('segment',
                                                   k16[:, :, 0], k16[:, :, 1], kf16_thin[:, 1], kf8[:, 1], kf4[:, 1],
                                                   k16[:, :, 0:1], ref_v, Ms[:, 0])
                ...
                this_logits, this_mask = self.STCN('segment',
                                                   k16[:, :, 1], k16[:, :, 2], kf16_thin[:, 2], kf8[:, 2], kf4[:, 2],
                                                   k16[:, :, 0:2], values, Ms[:, 1])
                ...
                # Ms[:,0]  sec_Ms[:,0]
                # Segment frame 1 with frame 0  k16[:,:,1]第一帧的key[4, 64, 24, 24], k16[:,:,0:1]:[4, 64, 1, 24, 24]
                prev_logits, prev_mask = self.STCN('segment',
                                                   k16[:, :, 0], k16[:, :, 1], kf16_thin[:, 1], kf8[:, 1], kf4[:, 1],
                                                   k16[:, :, 0:1], ref_v, Ms[:, 0],
                                                   sec_Ms[:, 0], selector)  # selector: [[1,0],[1,1],[1,1],[1,1],]
                ...
                # [4, 2, 512, 2, 24, 24] 2: objects,,  2:frames
                del ref_v

                # Segment frame 2 with frame 0 and 1
                this_logits, this_mask = self.STCN('segment', k16[:, :, 1],
            ...
            if self._is_train:
                if (it) % self.report_interval == 0 and it != 0:
                    if self.logger is not None:
                        ...
                        self.logger.log_metrics('train', 'time', (time.time() - self.last_time) / self.report_interval,
                                                it)
                    self.last_time = time.time()
                    self.train_integrator.finalize('train', it)
                    self.train_integrator.reset_except_hooks()
                    # writer.add_scalar('loss', losses['total_loss'], it)
                if self.para['stage'] == 1 and it % self.save_model_interval == 0 and it > 150000:
                    if self.logger is not None:
                        self.save(it)
                if self.para['stage'] == 0 and it % 50000 == 0:
                    if self.logger is not None:
                        self.save(it)
